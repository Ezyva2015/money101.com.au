...
class OptionsFramework_Backup {
	...
	function register_admin_screen () {
		...
		$this->admin_page = add_theme_page(__( 'Options Framework Import / Export', 'organizedthemes' ), __( 'Options Import / Export', 'organizedthemes' ), 'edit_theme_options', $this->token, array( &$this, 'admin_screen' ) );
...
?>
	<div class="wrap">
		...
		<h2><?php _e( 'Import / Export', 'organizedthemes' ); ?></h2>
		
		<div class="import">
			<h3><?php _e( 'Import Settings', 'organizedthemes' ); ?></h3>
			
			<p><?php _e( 'If you have settings in a backup file on your computer, the Import / Export system can import those into this site. To get started, upload your backup file to import from below.', 'organizedthemes' ); ?></p>
				...
				<form enctype="multipart/form-data" method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
					...
					<label for="OptionsFramework-import-file"><?php printf( __( 'Upload File: (Maximum Size: %s)', 'organizedthemes' ), ini_get( 'post_max_size' ) ); ?></label>
					...
					<input type="submit" class="button" value="<?php _e( 'Upload File and Import', 'organizedthemes' ); ?>" />
				</form>

		</div>
		
		<div class="export">
			<h3><?php _e( 'Export Settings', 'organizedthemes' ); ?></h3>
			
			<p><?php _e( 'When you click the button below, the Import / Export system will create a text file for you to save to your computer.', 'organizedthemes' ); ?></p>
			<p><?php echo sprintf( __( 'This text file can be used to restore your settings here on "%s", or to easily setup another website with the same settings".', 'organizedthemes' ), get_bloginfo( 'name' ) ); ?></p>
			...
			<form method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
				...
				<input type="submit" class="button" value="<?php _e( 'Download Export File', 'organizedthemes' ); ?>" />
			</form>
		</div>
...
//	} // End admin_screen_help()
	...
	function admin_notices () {
		...
		echo '<div id="import-notice" class="updated"><p>' . sprintf( __( 'Please note that this backup manager backs up only your theme settings and not your content. To backup your content, please use the %sWordPress Export Tool%s.', 'organizedthemes' ), '<a href="' . admin_url( 'export.php' ) . '">', '</a>' ) . '</p></div><!--/#import-notice .message-->' . "\n";
			
		if ( isset( $_GET['error'] ) && $_GET['error'] == 'true' ) {
			echo '<div id="message" class="error"><p>' . __( 'There was a problem importing your settings. Please Try again.', 'organizedthemes' ) . '</p></div>';
		} else if ( isset( $_GET['error-export'] ) && $_GET['error-export'] == 'true' ) {  
			echo '<div id="message" class="error"><p>' . __( 'There was a problem exporting your settings. Please Try again.', 'organizedthemes' ) . '</p></div>';
		} else if ( isset( $_GET['invalid'] ) && $_GET['invalid'] == 'true' ) {  
			echo '<div id="message" class="error"><p>' . __( 'The import file you\'ve provided is invalid. Please try again.', 'organizedthemes' ) . '</p></div>';
		} else if ( isset( $_GET['imported'] ) && $_GET['imported'] == 'true' ) {  
			echo '<div id="message" class="updated"><p>' . sprintf( __( 'Settings successfully imported. | Return to %sTheme Options%s', 'organizedthemes' ), '<a href="' . admin_url( 'admin.php?page=options-framework' ) . '">', '</a>' ) . '</p></div>';
		} // End IF Statement
...
/**
 * Create $OptionsFramework_Backup Object.
 *
 * @since 1.0.0
